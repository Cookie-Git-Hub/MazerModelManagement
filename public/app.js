document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(`https://5698-93-190-138-199.ngrok-free.app/validate`, {
...
function handleFileSelect(event) {
  const MAX_FILES = 8; // Максимальное количество файлов
  const MAX_TOTAL_SIZE_MB = 50; // Максимальный общий размер в МБ
  const MAX_TOTAL_SIZE_BYTES = MAX_TOTAL_SIZE_MB * 1024 * 1024;

  const files = Array.from(event.target.files);
  let totalSize = selectedFiles.reduce((sum, file) => sum + file.size, 0); // Суммарный размер уже выбранных файлов
